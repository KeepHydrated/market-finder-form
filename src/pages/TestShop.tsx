...
export default function TestShop() {
  console.log("TestShop component rendering...");
  ...
  useEffect(() => {
    ...
    if (!isSavingRef.current && !loadingShop) {
      ...
      if (newSection !== currentSection) {
        ...
        navigate(`/test?section=${newSection}${marketParam}`, { replace: true });
      }
        ...
        return shopData ? (
          <div className="space-y-6 ml-4 sm:ml-52 mr-4 sm:mr-8 max-w-5xl pt-10 sm:pt-[40px] pb-4">
            <BusinessCards
              ...
              vendorId={shopData.id}
              markets={selectedFarmersMarkets}
            />
          </div>
        ) : renderShop();
      case 'account':
  ...
  const renderOverviewWithTabs = () => (
    <div className="space-y-6 ml-4 sm:ml-52 mr-4 sm:mr-8 max-w-7xl pt-10 sm:pt-[40px] pb-4">
      {renderTest()}
    </div>
  );
          ...
          <div className="pt-6 border-t flex gap-4">
            {!shopData && (
              <Button 
                onClick={() => {
                  const marketParam = prefilledMarketId ? `&market=${prefilledMarketId}` : '';
                  navigate(`/test?section=products${marketParam}`);
                }}
                className="flex-1"
              >
  ...
  const renderProducts = () => (
    <div className="space-y-6 ml-4 sm:ml-52 mr-4 sm:mr-8 max-w-3xl pt-20 sm:pt-[40px] pb-4">
  ...
  const renderAccount = () => (
    <div className="space-y-6 ml-4 sm:ml-52 mr-4 sm:mr-8 max-w-3xl pt-10 sm:pt-[40px] pb-4">
      <div>
